...
export const HostDashboard: React.FC<HostDashboardProps> = ({
  ...
  const [discussionTime, setDiscussionTime] = useState<number>(30);
  const [votingTime, setVotingTime] = useState<number>(35);
  ...
  const handleStartGame = async () => {
    ...
    try {
      ...
      // 1. Sortear Impostores e Tripulantes
      ...
      if (isValidUuid) {
        // 2. Atualizar o papel (role) de cada jogador no Supabase DB (se o ID for um UUID válido)
      ...
      const payload = {
        status: "PLAYING",
        roles: rolesMap,
        rules: { killCooldown, impostorCount, taskCount, discussionTime, votingTime },
        timestamp: Date.now(),
      };
    ...
    <div className="relative w-full max-w-md mx-auto min-h-[85vh] bg-slate-950 text-white rounded-3xl overflow-hidden border-2 border-slate-800 shadow-2xl flex flex-col justify-between p-4 sm:p-5 select-none font-sans">
      ...
      <main className="z-10 my-4 space-y-4 flex-1 flex flex-col justify-between">
              ...
              <button
                ...
                onClick={() => {
                  const bot1: Player = {
                    id: `bot_${Date.now()}_1`,
                    room_id: roomId,
                    player_name: `Tripulante Azul (Bot)`,
                    color_hex: '#3b82f6',
                    status: 'ALIVE',
                    role: 'CREWMATE',
                  };
                  const bot2: Player = {
                    id: `bot_${Date.now()}_2`,
                    room_id: roomId,
                    player_name: `Tripulante Amarelo (Bot)`,
                    color_hex: '#eab308',
                    status: 'ALIVE',
                    role: 'CREWMATE',
                  };
                  setPlayers((prev) => [...prev, bot1, bot2]);
                }}
                className="px-2.5 py-0.5 rounded-lg bg-cyan-950/80 hover:bg-cyan-900 border border-cyan-800 text-[10px] font-bold text-cyan-300 uppercase tracking-wider transition-all active:scale-95 cursor-pointer"
                title="Adicionar 2 bots de teste para simular partida completa"
              >
                + Bots de Teste
              </button>
        ...
        <div className="bg-slate-900/90 border border-slate-800 p-4 rounded-3xl space-y-4 shadow-xl">
          ...
          {/* Duração da Discussão (Reunião Presencial) */}
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between text-xs font-semibold">
              <span className="text-slate-300 flex items-center gap-1.5">
                <MessageSquare className="w-4 h-4 text-purple-400" />
                Discussão & Deslocamento
              </span>
              <span className="text-purple-400 font-mono font-extrabold bg-purple-950/60 border border-purple-800/60 px-2.5 py-0.5 rounded-lg text-xs">
                {discussionTime >= 60
                  ? `${discussionTime}s (${Math.floor(discussionTime / 60)}m${discussionTime % 60 ? ` ${discussionTime % 60}s` : ""})`
                  : `${discussionTime}s`}
              </span>
            </div>
            <input
            ...
            <p className="text-[10px] text-slate-500 italic">
              Tempo para todos irem ao ponto de encontro antes da votação abrir.
            </p>
          </div>
          ...
          <div className="space-y-1.5 pt-1">
            <div className="flex items-center justify-between text-xs font-semibold">
              ...
              <span className="text-cyan-400 font-mono font-extrabold bg-cyan-950/60 border border-cyan-800/60 px-2.5 py-0.5 rounded-lg text-xs">
                {votingTime >= 60
                  ? `${votingTime}s (${Math.floor(votingTime / 60)}m${votingTime % 60 ? ` ${votingTime % 60}s` : ""})`
                  : `${votingTime}s`}
              </span>
            </div>
            <input
        ...
        >
          {isStarting ? (
            ...
            <>
              ...
              <span>Aguardando Conexão de Jogadores...</span>
            </>
          ) : (
            <>
              <Play className="w-5 h-5 text-emerald-200 fill-emerald-200" />
              <span>Iniciar Partida ({players.length} Jogador{players.length !== 1 ? 'es' : ''})</span>
            </>
          )}
        </button>
      </footer>
